...
void main()
{
	...
	position.x -= direction.x*slow*accel*t*t;
	position.y -= direction.y*slow*accel*t*t; // Particle moves up
	position.z += direction.z*slow*accel*t*t;
	...
	float distX = abs(position.x - center.x) / 3.0;
